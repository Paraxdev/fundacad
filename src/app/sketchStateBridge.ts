...
export function installSketchStateBridge(e: Engine): void {
  ...
  e.sketch.onState = () => {
    if (e.sketch.active && !sketchWasActive) e.ui.palette.emitAll(); // apply palette opts
    ...
    e.ui.palette.setVisible(e.sketch.active);
  ...
  e.ui.palette.onToggle = (key, value) => {
    switch (key) {
  ...
  e.ui.palette.onLookAt = () => e.sketch.lookAt();
}
